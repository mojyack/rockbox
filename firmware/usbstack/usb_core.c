...
struct usb_ctrlrequest handling_request;
struct usb_ctrlrequest pending_request;
volatile bool have_pending_request;
bool usb_debug = true;
...
unsigned char usb_control_data[256] USB_DEVBSS_ATTR;
    ...
    struct usb_transfer_completion_event_data* event)
{
    ...
    if(num == EP_CONTROL) {
        ...
        usb_core_control_request_handler(event->req);
        return;
    }
    ...
    for(int epnum = 0; epnum < USB_NUM_ENDPOINTS; epnum += 1) {
        for(int dir = 0; dir < 2; dir += 1) {
            ...
            if(init) {
                ...
                usb_drv_ep_init(&cstate->ep_alloc_ctx, ep, astate->type[dir], ps);
...
static bool usb_drv_ep_allocate(struct usb_drv_ep_alloc_ctx* ctx, int ep, int type) {
    ...
    for(int i = 0; i < USB_NUM_DRIVERS; i++) {
        ...
        for(int reqnum = 0; reqnum < driver->ep_allocs_size; reqnum += 1) {
            ...
            for(int epnum = 1; epnum < USB_NUM_ENDPOINTS; epnum += 1) {
                ...
                if(!usb_drv_ep_allocate(&cstate->ep_alloc_ctx, ep, req->type)) {
                    continue;
                }
    ...
    for(i = 0; i < USB_NUM_DRIVERS; i++) {
        ...
        if((req->bRequestType & USB_RECIP_MASK) == USB_RECIP_INTERFACE &&
            ...
            if(req->bRequest == USB_REQ_SET_INTERFACE) {
                ...
                if(alt >= 0 && alt < 255) {
                    usb_control_data[0] = alt;
                    usb_core_control_response(true, usb_control_data, 1);
                    handled = true;
                }
                break;
            }
        ...
        handled = driver->control_request(req);
...
static void request_handler_device_get_descriptor(struct usb_ctrlrequest* req)
{
        ...
        case USB_DT_CONFIG: {
            ...
            for(i = 0; i < USB_NUM_DRIVERS; i++) {
                if(drivers[i]->enabled && !drivers[i]->error && drivers[i]->config == index + 1 && drivers[i]->get_config_descriptor) {
                    size += drivers[i]->get_config_descriptor(&usb_control_data[size], max_packet_size);
                }
            }
            ...
            memcpy(&usb_control_data[0], &config_descriptor, sizeof(struct usb_config_descriptor));

            ptr = usb_control_data;
        } break;
        case USB_DT_STRING:
            logf("STRING %d", index);
    ...
    if(ptr) {
        ...
        if (ptr != usb_control_data)
            memcpy(usb_control_data, ptr, length);

        usb_core_control_response(true, usb_control_data, length);
    } else {
        usb_core_control_response(false, NULL, 0);
    }
}
...
static int usb_core_do_set_config(uint8_t new_config)
{
    ...
    static bool current_exclusive = false;
    if(!current_exclusive && require_exclusive) {
...
static void request_handler_device(struct usb_ctrlrequest* req)
{
    ...
    switch(req->bRequest) {
        ...
        case USB_REQ_GET_STATUS:
            usb_control_data[0] = 0;
            usb_control_data[1] = 0;
            usb_core_control_response(true, usb_control_data, 2);
            break;
        #ifdef USB_ENABLE_IAP
        case 0x40:
...
static void request_handler_endpoint_drivers(struct usb_ctrlrequest* req)
{
    bool handled = false;
...
static void request_handler_endpoint_standard(struct usb_ctrlrequest* req)
{
    switch (req->bRequest) {
        ...
        case USB_REQ_GET_STATUS:
            ...
            if(req->wIndex > 0)
                usb_control_data[0] = usb_drv_stalled(EP_NUM(req->wIndex),
                                                    EP_DIR(req->wIndex));

            usb_core_control_response(true, usb_control_data, 2);
            break;
        default:
            request_handler_endpoint_drivers(req);
            break;
    }
}
...
void usb_core_transfer_complete(int endpoint, int dir, int status, int length) {
    ...
    case USB_DIR_IN | EP0_EXPECT_RX_STATUS_COMP:
        ...
        break;
    default:
        logf("unhandled endpoint xfer completion ep0_state=%d dir=%d", ep0_state, dir);
        break;
    }
...
void usb_core_control_response(bool ack, const void* data, size_t size) {
    logf("usb_core: response ack=%d size=%u ep0_state=%d tick=%lu", ack, size, ep0_state, current_tick);
    ...
    case EP0_HANDLING_RX_CONTROL:
        ...
        break;
    default:
        logf("usb_core: invalid control response ep_state=%d", ep0_state);
        break;
    }
}
