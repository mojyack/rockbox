...
#endif
static long usb_stack[(DEFAULT_STACK_SIZE*6 + DUMP_BMP_LINESIZE + USB_EXTRA_STACK)/sizeof(long)];
...
{
    if(id == SYS_USB_CONNECTED_ACK) {
#ifdef HAVE_USBSTACK
        usb_core_ack_connection(revision);
#else
        queue_post(&usb_queue, id, revision);
#endif
    }
}
