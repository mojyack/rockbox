...
static void sink_set_sampr(uint16_t index) {
    cur_sampr = index;
    check_act(iap_select_sampr(&iap_ctx, samprs[cur_sampr]), );
...
static void batch_get_more(const void** ptr, size_t* len) {
    ...
    const size_t packet_size = calc_packet_size(cur_sampr, packet_count);
...
struct pcm_sink iap_pcm_sink = {
    ...
    .default_sampr_index = 0,
    .init                = sink_nop,
    .postinit            = sink_nop,
    .set_sampr           = sink_set_sampr,
    .lock                = sink_nop,
    .unlock              = sink_nop,
    .play                = sink_play,
    .stop                = sink_stop,
