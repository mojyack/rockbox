...
static int usb_hid_get_report(struct usb_ctrlrequest *req, unsigned char* dest)
{
    ...
    usb_core_control_response(1, dest, 2);
    return 0;
}
...
{
    unsigned char* dest = usb_control_data;
    unsigned char *orig_dest = dest;
    ...
    case USB_TYPE_STANDARD:
    {
        ...
        if (dest != orig_dest)
        {
            usb_core_control_response(1, orig_dest, dest - orig_dest);
            return true;
        }
        break;
    }
