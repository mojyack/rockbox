...
call `usb_core_control_response(true, NULL, 0)` for ACK or
`usb_core_control_response(false, NULL, 0)` for NAK.

If the request was a Control read transfer, the core or recipient class driver
will call `usb_core_control_response(true, data, size)` to send data packets,
or `usb_core_control_response(false, NULL, 0)` for NAK.
...
driver will process it, then `usb_core_control_response(true, NULL, 0)` for ACK
or `usb_core_control_response(false, NULL, 0)` for NAK.
