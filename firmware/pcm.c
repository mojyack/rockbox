                             ...
                             const void *addr, int count)
{
    ...
    if (active)
    {
        ...
        unsigned long sampr = sink->samprs[sink->configured_freq];
...
void pcm_init(void)
{
    ...
    for(size_t i = 0; i < ARRAYLEN(sinks); i += 1) {
        sinks[i]->pending_freq = sinks[i]->default_freq;
        sinks[i]->configured_freq = -1;
        sinks[i]->init();
    }
}
...
void pcm_set_frequency(unsigned int samplerate)
{
    ...
    if (samplerate != sink->samprs[index])
        index = sink->default_freq; /* Invalid = default */
...
{
    struct pcm_sink* sink = sinks[cur_sink];
    return sink->samprs[sink->pending_freq];
}
...
void pcm_apply_settings(void)
{
    ...
    if(sink->pending_freq != sink->configured_freq) {
        logf(" sink->set_freq");
        sink->set_freq(sink->pending_freq);
